...
- Reproduced the Startify-open startup race with the user's normal Vim
  ...
  discarded itself as stale once clangd reached vim-lsp's running state. A
  vim-lsp event was also replacing that special retry with the same stale
  ordinary debounce.
...
- Added a regression that combines the vim-lsp event with this stale startup
  snapshot, then confirms it retains automatic retry, requests, and rendering.
